...
//      * 3. Consulta para recuperar las hoteles con sus Hotels
...
// //ESTAS QUERIES NO ESTÁN BIEN, CORREGIR!!!

//     @Query(value = "select p from Hotel p left join fetch p.hotel") // NO es una consulta de mysql, sino HQL
//     public List<Hotel> findAll(Sort sort);

//     // Recuperar listado de Hotels paginados:

//     @Query(value = "select p from Hotel p left join fetch p.hotel", countQuery = "select count(p) from Hotel p left join p.hotel")
//     public Page<Hotel> findAll(Pageable pageable);
// //devuelve el numero de Hotels fijados por el pageable


//     // Consulta para recuperar las hoteles con sus Hotels utilizando HQL
//     // (Hibernate Quer Language), se recupera por el id:

//     @Query(value = "select p from Hotel p left join fetch p.hotel where p.id = :id")
